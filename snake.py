...
def handle_keys(direction):
    ...
    for event in [e for e in pygame.event.get() if e.type == pygame.KEYDOWN]:
        if event.key ==pygame.K_UP and direction != Direction.DOWN: # K= konstant
...
def get_food():
    global score
    print(food_position)
    if abs(snake_position[0] - food_position [0]) < 20 and abs (snake_position[1] - food_position[1]) < 15:
    #if abs(snake_position[1] - food_position [1]) < 20 and abs (snake_position[0] - food_position[0]) < 20:
...
def repaint():
    window.fill(pygame.Color(0,0,0)) # BLACK= 0,0,0
    for body in snake_body:
        pygame.draw.circle(window,pygame.Color(0,255,0),(body[0], body[1]), scale/2) # GREN =0,255,0 
    pygame.draw.rect(window, pygame.Color(255,0,0), pygame.Rect(food_position[0],food_position[1],scale, scale))  #RED 255,0,0

def game_over_message():
    print("__GAME OVER__")           
    font = pygame.font.SysFont("Arial", scale*3)   

    render = font.render(f" Score: {score}", True, pygame.Color(255,255,255),None) # WHITE= 255,255,255
    rect = render.get_rect()
    rect.midtop = (window_width/2, window_height/2)
    window.blit(render, rect)

    render1 = font.render(f"GAME OVER", True, pygame.Color(255,255,255),None) # WHITE= 255,255,255
    rect1 = render1.get_rect()
    rect1.midbottom = (window_width/2, window_height/2)
...
def game_over():
    ...
    if snake_position[1] < 0 or snake_position[1] > window_height -10:
        game_over_message()    
    for blob in snake_body[1:]:     # touch is body to the end
